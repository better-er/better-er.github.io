...
const TEXT = {
    ...
    LABEL_ORIGINAL: '原词：',
    LABEL_NEW: '新词：',
    MULTI_DESC: '逐行对齐显示（不匹配行将显示❗并统计）。对齐结果可配合「SV保留延音符粘贴歌词」脚本使用，粘贴到 Synthesizer V 时该脚本要求原词不可含延音符（-/+），需留意。适配中日英。\n\n分词规则：\n· 英语：按完整单词分词，连读/缩写中的 \' 不拆开（如 don\'t 会被视为一个整体，而非拆成 don 和 t）。\n· 日语：汉字/假名按单字符分词，但小假名（ぁぃぅぇぉゃゅょっ 等）会自动并入前一个字符，避免被拆成两个音节。建议原词用「一字一音」格式、新词用「全假名」格式。',
